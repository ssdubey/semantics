...
let resolve firstDistinctTreeHash lastkey_treehash lastkey restOfKeys branch blockstore tagstore =
  let treeNode = BlockMap.find firstDistinctTreeHash blockstore in
  match treeNode with
  | Value_tree (Tree treelist) -> 
    let tl = searchList lastKey treelist in (*removes the last key tuple from treelist if present, else return as it is*)
    let tl' = lastkey_treehash::tl in 
    let blockstore = BlockMap.add (Hash_tree tl') (Value_tree (Tree tl')) blockstore in
  ...
  |h::t -> 
    (let new_tree_node = [ (Node, h, prev_treehash) ] in
      if (String.compare h pivotKeyTerm == 0) then
        blockstore, (Hash_tree prev_treehash), t  (**returning tree node is the pivot tree node, 
...
let searchTreeList key treelist =
match treelist with
|[] -> None
|(c, p, v)::t ->
...
let searchKeyInTreelist k treelist =
    match treelist with
    |[] -> None
...
let rec prevToPivot key pivotKeyTerm tree_hash blockstore prev_to_Pivot =
    ...
    |Value_tree(Tree treelist) -> 
        ...
        |None -> None 
        |Some (c, p, v) -> 
            if (String.compare p pivotKeyTerm) then 
                prev_to_Pivot 
            else 
                prevToPivot (List.tl key) pivotKeyTerm v  blockstore (Some (c, p, v))


let searchAndReplaceTuple pivotKeyTerm nextTo_pivot_treehash treelist treelist' =
    match treelist with
        ...
        |Some (_, p, vh) -> (
            ...
            |Value_tree (Tree treelist) -> 
                ...
                in
                    ...
                    insertTopToPivotKeys key pivotKeyTerm (Hash_tree treelist') root_tree_hash blockstore
        )
        |None -> (
    ...
    if (String.compare (List.hd rev_key) pivotKeyTerm == 0) then
      ...
      let blockstore =
        BlockMap.add (Hash_tree new_tree_node) (Value_tree (Tree new_tree_node))
          blockstore
      in
        let blockstore, prevTo_pivot_treehash, restOfKeys = (**In [a;b;c;d;e] if d is pivot, treehash is of e*)
